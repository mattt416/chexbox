...
Table.prototype.getFieldWidth = function(field) {
  var str = '';
  ...
  for(let i=0; i<this.data.length; i++) {
    for(let k=0; k<this.heights[i]; k++) {
      ...
      for(let j=0; j<this.data[0].length; j++) {
        ...
        if (field[k]) {
          padding = this.widths[j] - this.getFieldWidth(field[k]);
          fieldRow = field[k];
        } else {
          fieldRow = '';
          padding = this.widths[j];
        }
